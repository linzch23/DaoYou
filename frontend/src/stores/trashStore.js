...
// 回收站域 Pinia store —— 唯一 owner of `trashedTrips` / `error` / `isFetching` / `restoringId` / `permanentlyDeletingId`
//
// Spec contract: specs/TrashPage.md §7.1
//
// state
//   trashedTrips          : TripSummary[]             已删行程列表(GET /api/trash/trips 后端过滤 deleted_at IS NOT NULL)
//   error                 : ApiError | null           拉取 / 恢复 / 永久删除失败时的错误
//   isFetching            : boolean                   fetchTrash 飞行中标记
//   restoringId           : number | null             当前正在恢复的 trip id(单值,防并发)
//   permanentlyDeletingId : number | null             当前正在永久删除的 trip id(单值,防并发)
//
// action
//   fetchTrash()                   : Promise<void>   GET /api/trash/trips → listDeletedTrips → 写 trashedTrips
//   restoreTrashById(id)           : Promise<void>   POST /api/trash/trips/{id}/restore + 乐观更新 + 失败回滚
//   permanentlyDeleteTrip(id)      : Promise<void>   DELETE /api/trash/trips/{id} + 乐观更新 + 失败回滚
...
//   - trashStore 持有 deleted_at!==null 删除行(per spec §1 决策:不污染 homeStore)
//
...
import {
  ...
  restoreTrashById as svcRestoreTrashById,
  permanentlyDeleteTrip as svcPermanentlyDeleteTrip,
...
export const useTrashStore = defineStore('trash', () => {
  ...
  /** @type {import('vue').Ref<number | null>} */
  const permanentlyDeletingId = ref(null)

  // ───────── Actions ─────────

  /**
   * 拉取已删行程列表 —— GET /api/trash/trips(后端过滤 deleted_at IS NOT NULL)
   *
   * v0.2.0 关键决策(per TrashPage spec §6.4.1 Resolved):
   *   - URL 改 `/api/trash/trips`(原 `/api/trips` 全量 + JS filter 路径废弃,后端已补端点)
   *   - service 层**不**做客户端 filter / sort(后端已做)
   *   - trashedTrips 内的 TripSummary 项 `deleted_at` 字段必非 null(后端保证)
   *
  ...
  /**
   * 恢复某条已删行程 —— 乐观更新 + POST /restore + 失败回滚协议(per spec §7.3 + §6.4.1 Resolved)
   *
   * v0.2.0 关键决策(per TrashPage spec §6.4.1 Resolved):
   *   - 调 `services/trips.restoreTrashById(tripId)`(POST /api/trash/trips/{id}/restore,后端置 deleted_at = null)
   *   - **不**再走 v0.1.0 `PUT /api/trips/{id} { status: 'active' }` 路径(原路径依赖 TripStatus 4 枚举含 'deleted')
   *
   * 实现要点(spec §7.3):
   *   1. findIndex 提前取 idx(因乐观更新会改 state.trashedTrips 引用,**不能**延后取)
   *   2. 乐观更新:trashedTrips.filter(t => t.id !== tripId) + restoringId = tripId
   *   3. await services/trips.restoreTrashById(tripId)
  ...
  async function restoreTrashById(tripId) {
    ...
    try {
      // 2. 调 services/trips.restoreTrashById(POST /api/trash/trips/{id}/restore)
      await svcRestoreTrashById(tripId)
  ...
  /**
   * 永久删除某条已删行程 —— 乐观更新 + DELETE + 失败回滚协议(per spec §7.4 + §6.4.2 Resolved)
   *
   * v0.2.0 关键决策(per TrashPage spec §6.4.2 Resolved,后端补 DELETE 端点):
   *   - 调 `services/trips.permanentlyDeleteTrip(tripId)`(DELETE /api/trash/trips/{id},后端真删)
   *   - **不**再走 v0.1.0 MVP 0 API 路径(等后端 30 天定时任务)
   *
   * 实现要点(spec §7.4):
   *   1. findIndex 提前取 idx(因乐观更新会改 state.trashedTrips 引用)
   *   2. 乐观更新:trashedTrips.filter(t => t.id !== tripId) + permanentlyDeletingId = tripId
   *   3. await services/trips.permanentlyDeleteTrip(tripId)
   *   4. 成功:permanentlyDeletingId = null + logger.info,row 已在乐观更新时移除
   *   5. 失败(非 404):回滚 splice(idx, 0, trip) + permanentlyDeletingId = null + error = err + throw
   *   6. 404/4001 静默:trip 已被自动清理,不回滚不切 error
   *
   * 错误归一(ApiError 复用):
   *   - 4000 / 400     → 4xx 业务错误
   *   - 4001 / 404     → 静默(trip 已不存在)
   *   - 5000 / 5xx     → 服务端错误
   *   - isNetworkError → 网络断开
   *
   * @param {number} tripId
  ...
  async function permanentlyDeleteTrip(tripId) {
    const idx = trashedTrips.value.findIndex((t) => t.id === tripId)
    if (idx === -1) {
      logger.warn('[trashStore.permanentlyDeleteTrip] target not found, skip', { tripId })
      return
    }
    const trip = trashedTrips.value[idx]

    // 1. 乐观更新:从 trashedTrips 移除 + 置 permanentlyDeletingId
    trashedTrips.value = trashedTrips.value.filter((t) => t.id !== tripId)
    permanentlyDeletingId.value = tripId
    error.value = null

    try {
      // 2. 调 services/trips.permanentlyDeleteTrip(DELETE /api/trash/trips/{id})
      await svcPermanentlyDeleteTrip(tripId)

      // 3. 成功:清空 permanentlyDeletingId
      permanentlyDeletingId.value = null
      logger.info('[trashStore.permanentlyDeleteTrip] ok', { tripId })
    } catch (err) {
      // 4. 失败:先看是不是 404(trip 已被自动清理)
      const apiErr = err instanceof ApiError
        ? err
      ...
      if (apiErr.code === 4001 || apiErr.statusCode === 404) {
        // 404 静默路径:不回滚不切 error 态
        permanentlyDeletingId.value = null
        logger.warn('[TrashPage] permanent delete failed, trip 404, dropping silently', { tripId })
        return
      }
      ...
      permanentlyDeletingId.value = null
      error.value = apiErr
      logger.error('[trashStore.permanentlyDeleteTrip] failed', { tripId, err })
      throw apiErr
    }
  }
   ...
   * 避免下次进入页面看到上次残留(trashedTrips / restoringId / permanentlyDeletingId 是删除行,数据可能频繁变化)
   */
  function clearTrash() {
    trashedTrips.value = []
    error.value = null
