...
| 成员 | 角色 | 核心负责内容 |
| --- | --- | --- |
| 成员 A | 前端负责人 | `frontend/`、首页、行程页、AI 对话页、拍照讲解页、我的页面、UI 优化 |
| 成员 B | 后端负责人 | FastAPI、API 设计、行程模块、用户模块、PostgreSQL 连接 |
...
feat(agent): add replan graph node
...
| 成员 | 任务 |
| --- | --- |
| 成员A | 开发首页时间轴和行程页，先接 Mock 数据 |
| 成员B | 实现 trip、trip_day、trip_item API |
| 成员C | 实现 Agent 读取行程上下文方法 |
| 成员D | 准备“大连三日游”测试数据，测试行程 API |
...
| 成员 | 任务 |
| --- | --- |
| 成员A | 开发提醒展示、改线草案确认弹窗、首页刷新 |
| 成员B | 实现 `/api/reminders/check`、`/api/reminders`、`/api/trips/{trip_id}/replan`、`/apply-plan` |
| 成员C | 实现 reminder node、replan node、地图工具封装、提醒 Prompt |
| 成员D | 准备时间冲突和出发提醒测试数据，测试改线后数据库更新 |
...
| #7 complete trip module | 成员A、B、D | 行程 API、首页、行程页、测试数据 | 首页展示今日行程 |
...
| #10 complete replanning module | 成员A、B、C | replan API、replan node、确认弹窗 | 动态改线可演示 |
...
| 第4周 | 提醒 UI、改线确认弹窗 | reminder/replan API | reminder/replan node | 风险和改线测试数据 | 智能提醒和动态改线可演示 |
