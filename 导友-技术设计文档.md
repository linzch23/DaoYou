...
| app/api/ | REST API 路由，例如 trips、chat、photos、reminders |
| app/core/ | 配置、日志、鉴权、异常处理 |
| app/db/ | 数据库连接、Session、迁移相关配置 |
| app/models/ | SQLAlchemy ORM 模型 |
| app/schemas/ | Pydantic 请求和响应模型 |
| app/services/ | 业务服务层，封装行程、文件、偏好等逻辑 |
...
| current_location | dict | 当前经纬度、城市或手动选择位置 |
...
| replan_node | message, trip, preferences | final_response | 生成替代行程草案 |
...
| 成员 | 角色 | 负责范围 | 具体任务 |
| --- | --- | --- | --- |
| 成员 A | 前端负责人 | `frontend/` | UniApp 页面、首页、行程页、AI 对话页、拍照讲解页、我的/偏好页、UI 美化 |
| 成员 B | 后端基础与行程模块负责人 | `backend/app/api`, `services` | FastAPI 项目、uv、PostgreSQL 连接、行程 API、首页 API、REST API 规范 |
...
| 第 4 周 | 主动能力 | 智能提醒、动态改线 | 可检查风险，可生成改线草案并应用 |
...
gantt
    ...
    section 第2周
    行程管理 API                   :b1, 2026-06-08, 4d
    UniApp 首页与行程页            :b2, 2026-06-08, 5d
