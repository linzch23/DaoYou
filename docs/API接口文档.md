...
只选择日期覆盖查询日且状态为 `active` 的旅行；若同一天存在多个符合条件的旅行，
返回 ID 最大（最新创建）的旅行。
...
上传用户最新位置。Android 客户端首次启动时申请定位权限，授权成功后立即调用一次；
恢复前台时再次调用，前台和有效行程后台运行期间约每 15 分钟调用一次。
...
用户确认表单后，在单个数据库事务中创建旅行、行程日和行程节点。每个节点由后端使用
`city + address/title` 调用高德地理编码，全部坐标解析成功后才提交；任一节点解析失败则整单
回滚并返回 HTTP 422。请求不接受 `latitude`、`longitude`。`idempotency_key` 用于防止网络
重试造成重复创建；同一用户重复提交相同键时返回原 `trip_id`。
