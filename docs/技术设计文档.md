...
+ 首次安装若定位权限尚未决定，App 主动触发系统授权；“我的-定位与后台提醒”展示
  权限和服务状态，并提供打开应用详情设置和重新启动入口。
+ Android 有效行程期间运行原生 location 前台服务，WebView 暂停后仍直接调用 `PUT /api/location`。
+ 独立常驻 `reminder-worker` 对齐自然季度点，每 15 分钟扫描一次。
+ 首页与 Worker 只处理状态为 `active` 且日期覆盖当天的旅行；同一天有多个符合条件的
  旅行时统一选择 ID 最大（最新创建）的一条。
...
+ 新建节点和 `/api/trips/from-draft` 批量创建的全部节点只有在地理编码成功后才入库；
  批量创建中任一地点解析失败会回滚整个事务。修改城市、地点名称或地址时重新解析坐标并清除旧到达状态。
...
功能：获取今日行程。接口不返回站内提醒或未读数量。同一天存在多个符合条件的
`active` 旅行时返回 ID 最大（最新创建）的旅行。
