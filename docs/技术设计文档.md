...
**场景四：智能提醒**  
MVP 阶段采用半主动提醒。用户点击“检查行程风险”，系统读取下一站时间和当前安排，判断是否存在出发时间、闭馆时间或时间冲突风险，并生成提醒。
...
### 2.5 数据库职责
PostgreSQL 存储长期业务数据，包括用户、旅行、行程日、行程项、聊天消息、用户偏好、用户记忆、照片记录和提醒记录。
...
### 6.5 智能提醒模块
MVP 阶段采用“半主动提醒”：用户点击“检查行程风险”，系统自动分析是否需要提醒。这样可以在不接入复杂后台推送的情况下展示主动 Agent 能力。
...
```mermaid
flowchart TD
    A[用户点击检查行程风险] --> B[读取今日行程]
    B --> C[读取当前时间]
    C --> D[读取下一行程项]
    D --> E[调用 map_tool 估算路程]
    E --> F[判断是否来得及]
    F --> G{存在风险?}
    G -->|否| H[返回暂无风险]
    G -->|是| I[生成提醒文案]
    I --> J[保存 notifications]
    J --> K[前端展示提醒]
```
...
MVP 可简化登录系统，初始化一个默认用户 `user_id=1`，方便团队优先完成核心功能。
...
| #11 complete reminder module | 完成风险检查和提醒列表 | 半主动提醒可演示 |
...
| 扩展方向 | 说明 |
| --- | --- |
| 真实后台定时推送 | 使用 Celery / APScheduler + 厂商推送实现真正主动提醒 |
