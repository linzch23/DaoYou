...
+ 提醒中心展示系统生成的出发提醒、时间冲突提醒等。
+ Android App 获得定位权限后，每 15 分钟通过 `PUT /api/location` 上报最新位置。
...
### 2.5 数据库职责
PostgreSQL 存储长期业务数据，包括用户及其最新位置、旅行、行程日、行程项、聊天消息、用户偏好、用户记忆、照片记录和提醒记录。用户位置只保存最新快照，不保存历史轨迹。
...
+ 提醒检查结果直接写入 `reminders` 表，避免依赖额外缓存服务。
+ Android 上报的最新位置直接写入 `users`；通过采集时间防止乱序请求覆盖新位置。
+ 大模型响应慢或外部 API 异常时，通过本地 fallback 文案和固定演示数据兜底。
...
| app/api/ | REST API 路由，例如 trips、trash、chat、photos、reminders |
...
| trip_id | int | 当前旅行 ID；仅提醒检查、行程工具和改线应用等行程相关流程需要 |
| user_message | str | 用户输入文本 |
| current_trip | dict | 当前旅行、行程日和行程项；普通聊天和拍照讲解可为空对象 |
...
| 节点名称 | 输入 | 输出 | 职责 |
| --- | --- | --- | --- |
| load_context_node | user_id, trip_id | current_trip, chat_history | 按场景读取行程上下文和用户级聊天历史 |
...
flowchart TD
    ...
    B -->|reminder_check| E[reminder_node]
...
+ 已进入回收站的旅行不能用于首页、提醒、改线和行程上下文。聊天和拍照讲解为用户级能力，不再依赖旅行 ID。
+ 永久删除和清空回收站会删除旅行及其行程日、行程节点、提醒等旅行级关联数据，操作不可恢复；用户级聊天和照片讲解历史不随旅行删除。
+ 本阶段不实现 30 天自动删除。
...
+ 用户点击“检查行程风险”时，可显式提交当前位置；未提交时读取用户表中的最新有效位置。
+ Android App 每 15 分钟上报用户最新位置。
+ `reminder-worker` 定时读取未来行程和用户最新位置，生成提醒并交给推送模块。
+ 位置采集时间距检查时间超过 30 分钟时视为过期，不用于精确路线时间计算。
...
flowchart TD
    ...
    C[用户手动检查] --> D[读取请求位置或 users 最新位置]
    E[reminder-worker 定时执行] --> F[读取 users 最新位置]
    B --> F
    D --> G{位置是否在 30 分钟内}
    F --> G
    G -->|是| H[调用 map_tool 估算路程]
    G -->|否| I[跳过依赖精确起点的计算]
    H --> J[结合行程时间判断风险]
    I --> J
    J --> K{存在风险?}
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| id | BIGSERIAL PK | 提醒 ID |
| user_id | BIGINT FK | 用户 ID |
| trip_id | BIGINT FK | 旅行 ID |
| type | VARCHAR(50) | departure / conflict / weather / rest |
| content | TEXT | 提醒内容 |
...
+ 一个旅行包含多个行程日。
+ 一个行程日包含多个行程节点。
+ 聊天消息和照片记录是用户级历史，不关联具体旅行。提醒记录仍关联旅行，用于首页未读提醒、提醒列表和旅行永久删除时级联清理。
...
+ 永久删除旅行时，`trip_days`、`trip_items` 和 `reminders` 通过外键级联删除。
+ 聊天记录、照片讲解记录和 `uploads/images/` 中的用户级图片不随旅行删除；后续如需清理，应设计独立的用户级历史删除接口。
...
功能：手动检查当前行程风险。请求未携带 `current_location` 时，后端读取 `users` 中 30 分钟内的最新位置；后台 `reminder-worker` 使用同一有效性规则。
...
请求参数：`user_id`, `trip_id`

### 8.10 对话内动态改线
动态改线复用 `POST /api/chat`，不再提供 `/replan` 和 `/apply-plan` 接口。

Agent 识别到 `intent = "replan"` 时，在 Chat 响应中返回 `action_options`。每个选项包含 `operation = "update_trip_item"`、目标 `item_id` 和可直接提交给行程节点更新接口的 `payload`。Chat 流程不修改数据库；用户确认后由调用方补充 `user_id`，再调用 `PUT /api/trip-items/{item_id}`。
...
| #11 complete reminder module | 完成位置上报、风险检查、提醒列表和后台监测 | 主动提醒可演示 |
| #12 ui optimization | 优化首页、聊天页、拍照页和提醒 UI | 提升复赛观感 |
