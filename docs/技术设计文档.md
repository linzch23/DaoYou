...
+ 提醒检查结果直接写入 `notifications` 表，避免依赖额外缓存服务。
...
| 节点名称 | 输入 | 输出 | 职责 |
| --- | --- | --- | --- |
| load_context_node | user_id, trip_id | current_trip, chat_history | 读取当前行程和历史消息 |
...
+ 已进入回收站的旅行不能用于首页、聊天、拍照讲解、提醒、改线和 Agent 上下文。
+ 永久删除和清空回收站会删除旅行及其关联数据，操作不可恢复。
+ 本阶段不实现 30 天自动删除。
...
+ 永久删除旅行时，`trip_days`、`trip_items`、`chat_messages`、`photo_records` 和 `notifications` 通过外键级联删除。
+ 删除 `photo_records` 后还需要由回收站服务安全清理 `uploads/images/` 中的关联文件。
