...
| 成员 | 角色 | 核心负责内容 |
| --- | --- | --- |
| 成员 A | 前端负责人 | `frontend/`、首页、行程页、回收站页、AI 对话页、拍照讲解页、Android 定位上报、我的页面、UI 优化 |
| 成员 B | 后端负责人 | FastAPI、API 设计、行程模块、回收站模块、用户位置模块、PostgreSQL 连接 |
...
| 成员 | 任务 |
| --- | --- |
| 成员A | 实现前台与 Android 原生后台定位、vivo regId 自动注册、Chat 改线交互 |
| 成员B | 实现 `PUT /api/location`、设备注册接口、独立 `reminder-worker`、高德驾车与 vivo Direct Push |
| 成员C | 维护 Chat 内改线意图分支和固定提醒文案；提醒规则不依赖大模型 |
| 成员D | 维护 users 位置字段、`device_push_tokens`、`departure_alerts` 和到达字段迁移及测试数据 |


验收：

+ Worker 按 15 分钟周期判断下一目的地并直接发送系统通知，不建设站内提醒列表。
+ Android 前台和原生前台服务可定期上报位置，旧时间戳不会覆盖新位置。
+ 超过 30 分钟的位置不会用于精确路线提醒。
...
| #11 complete reminder module | 成员A、B、C、D | location API、Android 前后台定位、设备注册、独立 Worker、vivo Direct Push | 主动提醒可演示 |
...
| 第4周 | 前后台定位、通知权限、Chat 改线选项交互 | location/push device API、Worker、Chat action options | reminder node、Chat 改线意图分支 | 位置、设备、提醒审计迁移及测试数据 | 系统通知和动态改线可演示 |
...
gantt
    ...
    Push设备与Worker       :b4, 2026-06-22, 5d
...
+ Prompt 输出格式可被后端解析。
+ 图片上传失败时有错误提示。
+ 配置真实 `LLM_API_KEY` 和 `QWEN_API_KEY` 后，chat/photo 返回内容不能是固定 fallback。
+ 前端拍照讲解请求必须携带 `tripId`，后端 `trip_id` 为必填。
...
+ Prompt 修改记录。
+ AI fallback 策略。
+ curl 联调记录：`/api/chat`、`/api/chat/history`、`/api/photos/explain`。
...
+ Docker Compose 已使用根目录 `.env` 注入后端环境变量。
+ Alembic head 已更新到 `20260623_0006`，chat/photo 数据表恢复 `trip_id`。
+ H5 构建已通过，CORS 预检已通过。
+ `/api/chat` 已验证真实 LLM 返回非固定回复。
+ `/api/photos/explain` 已验证 Qwen 视觉能识别上传的相机图标，而不是固定景点 fallback。
...
| 个性化讲解 | AI 回复能体现用户偏好 |
| 智能提醒 | Worker 能按 ETA 阈值生成 warning/critical 并通过 vivo 系统通知送达 |
| 用户位置 | Android 每 15 分钟可上报最新位置，后端拒绝旧时间戳覆盖，过期位置安全降级 |
