...
| 成员 | 任务 |
| --- | --- |
| 成员A | 开发提醒展示、Android 定位权限与每 15 分钟位置上报、Chat 改线选项选择交互、首页刷新 |
| 成员B | 实现 `PUT /api/location`、位置新旧校验、`/api/reminders/check`、`/api/reminders`，扩展 `/api/chat` 返回 `action_options` |
| 成员C | 实现 reminder node、Chat 内改线意图分支、地图工具封装、提醒和改线 Prompt |
| 成员D | 增加 users 位置字段迁移，准备新旧时间戳、过期位置和出发提醒测试数据，验证数据库状态 |


验收：

+ 点击检查风险后生成提醒。
+ Android 可定期上报位置，旧时间戳不会覆盖新位置。
...
| #11 complete reminder module | 成员A、B、C、D | location API、Android 定位上报、reminder API、提醒页、测试数据 | 主动提醒可演示 |
...
| 第4周 | 定位上报、提醒 UI、Chat 改线选项交互 | location/reminder API、Chat action options、节点更新接口 | reminder node、Chat 改线意图分支 | 位置迁移、过期位置和改线测试数据 | 智能提醒和动态改线可演示 |
